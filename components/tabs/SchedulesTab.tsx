...
export function SchedulesTab() {
  ...
  return (
    <div className="flex flex-col gap-3 h-full overflow-y-auto">
      {sorted.map((sc) => (
        <div key={sc.id}
          className={`bg-white border border-gray-200 shadow-sm rounded-xl p-4 flex items-start gap-3 transition-opacity ${!sc.enabled ? 'opacity-50' : ''}`}>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <span className="font-mono text-xl font-bold text-green-600 tabular-nums">{sc.time}</span>
              <span className="text-sm font-medium text-gray-800 truncate">{sc.label}</span>
            </div>
            <p className="text-xs text-gray-500 mt-1">{getLabel(sc.announcementRef)}</p>
            <div className="flex gap-1 mt-2 flex-wrap">
              {sc.days.map((d) => (
                <span key={d} className="text-xs bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded">{DAY_LABELS[d]}</span>
              ))}
              {sc.bgmAction !== 'none' && (
          ...
          <div className="flex gap-1 shrink-0 flex-col items-end">
            <button onClick={() => toggleSchedule(sc.id)}
              className={`px-2 py-1 text-xs rounded-lg transition-colors ${sc.enabled ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
              {sc.enabled ? '켜짐' : '꺼짐'}
            </button>
            <div className="flex gap-1">
              <button onClick={() => { setEditing(sc); setShowForm(true); }}
                className="px-2 py-1 text-xs bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition-colors">편집</button>
              <button onClick={() => deleteSchedule(sc.id)}
                className="px-2 py-1 text-xs bg-red-50 hover:bg-red-100 text-red-500 rounded-lg transition-colors">삭제</button>
            </div>
          </div>
        </div>
      ))}
...
function ScheduleForm({ initial, categories, onSave, onCancel }: {
  ...
  return (
    ...
    >
      <h3 className="font-semibold text-gray-800">{initial ? '스케줄 편집' : '새 스케줄'}</h3>
      ...
      <div className="flex flex-col gap-1">
        <label className="text-xs text-gray-500">안내방송</label>
        ...
        <div className="flex gap-1.5 flex-wrap">
          {allDays.map((day) => (
            <button key={day} type="button" onClick={() => toggleDay(day)}
              className={`w-9 h-9 rounded-lg text-sm font-medium transition-colors ${days.includes(day) ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>
              {DAY_LABELS[day]}
            </button>
          ))}
          <button type="button" onClick={() => setDays(allDays.slice(0,5))} className="px-2 py-1 text-xs bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-lg transition-colors">평일</button>
          <button type="button" onClick={() => setDays(['saturday','sunday'])} className="px-2 py-1 text-xs bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-lg transition-colors">주말</button>
          <button type="button" onClick={() => setDays(allDays)} className="px-2 py-1 text-xs bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-lg transition-colors">매일</button>
        </div>
      </div>
      <div className="flex gap-4 items-end">
        <div className="flex flex-col gap-1">
          <label className="text-xs text-gray-500">BGM 동작</label>
